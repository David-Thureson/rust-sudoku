...
// - Changed some data types to allow grids larger than 16 x 16.
...
fn try_build() {
    ...
    // 17 x 17.
    // let grid = Grid::build(17, 17, 4, 4, max_grid_count);

    // 18 x 18.
    // let grid = Grid::build(18, 18, 6, 3, max_grid_count);

    // 20 x 20.
    // let grid = Grid::build(20, 20, 5, 4, max_grid_count);

    // 25 x 25: 6 seconds.
    let grid = Grid::build(25, 25, 5, 5, max_grid_count);

    // 30 x 30: 17 seconds.
    // let grid = Grid::build(30, 30, 6, 5, max_grid_count);

    // 36 x 36: 58 seconds.
    // let grid = Grid::build(36, 36, 6, 6, max_grid_count);

    // 49 x 49.
    // let grid = Grid::build(49, 49, 7, 7, max_grid_count);
    ...
    pub fn new(grid_width: u8, grid_height: u8, block_width: u8, block_height: u8, max_tried_grid_count: Option<usize>) -> Self {
        ...
        */
        ...
        for index in 0..cell_count {
            let (row, col, block) = row_col_block(index as u16, grid_width, block_width, block_height, block_col_count);
    ...
    */
    ...
    fn print_simple_and_remaining_internal(&self, label: &str, print_remaining_counts: bool, print_remaining: bool) {
        ...
        for index in 0..self.cell_count as usize {
            ...
            let value = if value == 0 { ".".to_string() } else { value_to_char(value, self.max_value) };
            ...
            if print_remaining_counts {
                ...
                let value = if value == 0 { ".".to_string() } else { value_to_char(value as u8, self.max_value) };
    ...
    fn print_remaining_values(&self) {
        ...
        for index in 0..self.cell_count as usize {
            ...
            let internal_values = if cell_value > 0 {
                completed_template.replace("#", &value_to_char(cell_value, self.max_value))
            } else {
                let mut s = "".to_string();
...
*/
    ...
    fn cell_display(&self, index: u16) -> String {
        ...
        let values_string = if value == 0 {
            let remaining_values_string = self.remaining_values(index).iter().map(|value| value_to_char(*value, self.max_value)).collect::<Vec<_>>().join("");
            format!("remaining_values = [{}]", remaining_values_string)
        } else {
            format!("value = {}", value_to_char(value, self.max_value))
        };
...
fn value_to_char(value: u8, max_value: u8) -> String {
    if max_value <= 9 {

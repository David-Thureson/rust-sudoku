...
fn try_build() {
    ...
    let duration_limit = Some(Duration::from_millis(100_000));
    ...
    // let (grid, controller) = Grid::build(12, 12, 4, 3, max_tried_grid_count, duration_limit);
...
impl Grid {
    ...
    fn build_next_cell(controller: &mut BuildController, grid_to_now: &Grid) -> Option<Grid> {
        ...
        while !options.is_empty() {
            ...
            let remaining_options = grid.set_value(controller, option.0, *option.1);
            if remaining_options == 0 {
                ...
                let completed_grid = Self::build_next_cell(controller,&grid);
    ...
    */
    ...
    fn set_value(&mut self, controller: &mut BuildController, index: u8, value: u8) -> u16 {
        ...
        if self.unsolved_cell_count == 0 {
            ...
            //rintln!("set_value(): self.unsolved_cell_count == 0");
            return 0;
        }
        ...
        for related_cell_index in self.related_cell_mappings[index as usize].clone().iter() {
            ...
            if related_cell.value == 0 {
                ...
                if one_cell_remaining_options == 0 {
                    //rintln!("set_value(): one_cell_remaining_options == 0");
                    return 0;
                }
            }
        }
        ...
        controller.continue_branch = true;
        self.clear_single_remaining(controller);
        if !controller.continue_branch {
            // A cell was found with zero remaining options, so this branch won't work.
            //rintln!("set_value(): clear_single_remaining changed controller.continue_branch to false");
            return 0;
        }
    ...
    fn clear_single_remaining(&mut self, controller: &mut BuildController) {
        controller.log_event("clear_single_remaining()");
        loop {
            ...
            let remaining_after_set_value = self.set_value(controller, index, value);
            if remaining_after_set_value == 0 || !controller.continue_branch {

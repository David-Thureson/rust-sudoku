...
impl Grid {
    ...
    #[inline]
    pub fn clear_remaining_values(&mut self, index: u16) {
        if RUN_INVARIANT { self.invariant(); }
        for value in 1..=self.max_value {
            self.clear_remaining_value(index, value);
        }
    ...
    #[inline]
    fn index_to_related_cell_indexes(&self, index: u16) -> Vec<u16> {
                    ...
                    for related_cell_lookup_index in self.related_cell_range(index as u16) {
                        ...
                        if related_cell_index != index {
                            if self.values[related_cell_index] == value {
                                let error_message = format!("{} and {} have the same value {}.", self.cell_display(index as u16), self.cell_display(related_cell_index as u16), value);
                            ...
                            if self.remaining_values[remaining_value_index] {
                                let error_message = format!("{} has a remaining value found in {}.", self.cell_display(related_cell_index as u16), self.cell_display(index as u16));
                                self.print_simple_and_remaining(&error_message);
                                panic!(error_message);
                            }
                        }
